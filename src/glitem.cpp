    ...
    QQuickItem(parent)
{
    ...
    //m_eye = 10.0 * v_XYZ; //Entfernung
    //m_eye = 30.0 * v_Z;
    m_eye = 25.0 * QVector3D(0,-1,1);
    m_center = v_Zero-QVector3D(0,-3,0); //Verschiebung nach unten, da viewportY alles nach oben schiebt
    m_up = v_Y;
    //pespective
    m_fovy = 45.0;
    m_aspect = 1.0;
...
void GLItem::paintBefore()
{
 //   qDebug() << "GlItem::paintBefore() called";
    //initializeOpenGLFunctions(); jetzt in initializeRenderer()
...
void GLItem::toggleMove()
{
    ...
    if(m_timer->isActive()){
        m_timer->stop(); qDebug() << "m_timer->stop()";}
    else {m_timer->start(); qDebug() << "m_timer->start()";}
...
bool GLItem::setupRenderer()
{
    ...
    //m_renderer->rotate(m_renderThreadRotation, QVector3D(0.0,1.0,0.0)); Änderung Marvin
